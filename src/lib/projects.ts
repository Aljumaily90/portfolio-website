import { Project } from '@/types'

export const projects: Project[] = [
  {
    id: 'weeklymeals',
    name: 'WeeklyMeals',
    description: 'KI-gestützte Meal-Planning SaaS App. Generiert personalisierte Wochenpläne, Rezepte, Einkaufslisten und Zero-Waste-Empfehlungen. Verfügbar als PWA auf Web, iOS und Android.',
    url: 'https://www.weeklymeals.io/',
    type: 'pwa',
    tags: ['Next.js', 'PWA', 'KI', 'SaaS'],
    status: 'live',
  },
  {
    id: 'kraftlog',
    name: 'KraftLog',
    description: 'Fitness-Tracking PWA zum Erfassen von Trainingseinheiten, Wiederholungen und persönlichen Rekorden. Mit Streak-System und Level-Fortschritt — installierbar auf dem Homescreen.',
    url: 'https://pushup-app-seven.vercel.app/',
    type: 'pwa',
    tags: ['PWA', 'Fitness', 'Tracking'],
    status: 'live',
  },
  {
    id: 'mini-tagebuch',
    name: 'Mini-Tagebuch',
    description: 'Minimalistisches digitales Tagebuch für alle, die Notizen und Gedanken einfach und ohne Ablenkung festhalten möchten. Klares Design, schnelle Bedienung.',
    url: 'https://mini-tagebuch.vercel.app/',
    type: 'pwa',
    tags: ['React', 'Produktivität', 'Minimalismus'],
    status: 'live',
  },
  {
    id: 'ausgeliehen',
    name: 'Was habe ich ausgeliehen?',
    description: 'PWA zum Tracken verliehener Gegenstände. Nie wieder vergessen, wem du was geliehen hast — übersichtlich, offline-fähig und direkt auf dem Handy installierbar.',
    url: 'https://was-habe-ich-ausgeliehen.vercel.app/',
    type: 'pwa',
    tags: ['PWA', 'Produktivität', 'Offline'],
    status: 'live',
  },
  {
    id: 'techvs',
    name: 'Techvs',
    description: 'Interaktive Produktvergleichs-Plattform für Tech-Geräte. Nutzer setzen Prioritäten in 6 Kategorien und erhalten passende Empfehlungen für Smartphones, Notebooks, Tablets und mehr.',
    url: 'https://www.techvs.ch/',
    type: 'website',
    tags: ['Next.js', 'React', 'Produktvergleich'],
    status: 'live',
  },
  {
    id: 'dostour',
    name: 'Dostour',
    description: 'Moderne Website für ein Reisebüro, das Gruppenreisen, Individualreisen und Abenteuertouren in der Schweiz, der Türkei und Europa anbietet. Mit Buchungsformularen und Reisekatalogen.',
    url: 'https://www.dostour.ch/',
    type: 'website',
    tags: ['Next.js', 'React', 'Reise', 'Business'],
    status: 'live',
  },
  {
    id: 'eman-bestattungen',
    name: 'Eman Bestattungen',
    description: 'Professionelle Website für einen Bestattungsdienstleister, spezialisiert auf islamische Bestattungen in der Region Luzern und der ganzen Schweiz. Mehrsprachig (DE/EN/TR).',
    url: 'https://www.eman-bestattungen.ch/',
    type: 'website',
    tags: ['Responsive', 'Mehrsprachig', 'Business'],
    status: 'live',
  },
]
